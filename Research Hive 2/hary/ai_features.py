...
class AIFeatures:
    ...
    def find_research_gaps(self, papers, field_name):
        ...
        try:
            # Ensure field_name is not empty
            if not field_name or field_name.strip() == "":
                field_name = "this research area"
                
            logger.info(f"Analyzing research gaps in {field_name} field")
            
            # Ensure papers have abstracts
            valid_papers = []
            for paper in papers[:8]:  # Limit to 8 papers to avoid token limits
                if 'abstract' in paper and paper['abstract'] and len(paper['abstract']) > 50:
                    valid_papers.append(paper)
                elif 'title' in paper and paper['title']:
                    # If no abstract, we can still use the title
                    paper['abstract'] = f"Title: {paper['title']}"
                    valid_papers.append(paper)
            
            if len(valid_papers) < 3:
                logger.warning(f"Not enough valid papers with abstracts: {len(valid_papers)}")
                return None
                
            # Prepare paper data for analysis
            paper_data = []
            for i, paper in enumerate(valid_papers):
                # Get abstract, limit to 500 chars to avoid token limits
            ...
            IMPORTANT: Your analysis should be specific to these papers and the field of {field_name}. 
            Do NOT provide generic responses about AI or other fields unless they are directly relevant to {field_name}.
            ...
            response = self.client.chat.completions.create(
                model="llama3-8b-8192",
                messages=[
                    {"role": "system", "content": f"You are a research analyst specializing in identifying research gaps and opportunities in academic literature in the field of {field_name}. You analyze papers carefully and provide specific insights based on their content, not generic observations."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=800,
                temperature=0.5
            ...
            if not sections["research_gaps"] and not sections["emerging_trends"] and not sections["research_opportunities"]:
                logger.warning("No valid sections found in the analysis")
                return None
...
# Add this method to your AIFeatures class

def sort_papers_by_citations(self, papers):
    """
    Sorts a list of papers by citation count in descending order.
...
# Then modify any methods that return paper lists to use this sorting
# For example, if you have a method like:

def find_research_gaps(self, query, papers):
    # Your existing code to find research gaps
    
    # Before returning, sort the papers
    sorted_papers = self.sort_papers_by_citations(papers)
