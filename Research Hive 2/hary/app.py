...
def search():
    ...
    try:
        ...
        max_results = min(max(int(data.get('max_results', 10)), 1), 20)  # Ensure between 1 and 20
        ...
        # Perform search
        logger.info(f'Searching for papers with keywords: {keywords}, max_results: {max_results}, AI enabled: {enable_ai}')
        ...
        if result:
            ...
            if isinstance(result, dict) and 'papers' in result:
                ...
                logger.info(f'Found {len(papers)} papers and {len(related_topics)} related topics')
                ...
                return jsonify({
                    'success': False,
                    'error': 'Unexpected result format from search'
                })
        else:
        ...
        return jsonify({
            'success': False,
            'error': 'Invalid search parameters. Please try again.'
        })
...
def research_gaps():
    ...
    try:
        ...
        papers = data.get('papers') if 'papers' in data else session.get('last_search_papers')
        field = data.get('field') if 'field' in data else session.get('last_search_field', 'this research area')
        
        # If the request is empty, it might be just checking for session data
        if not data or (not 'papers' in data and not 'field' in data):
            # Return the session data without analysis
            return jsonify({
                'success': True,
                'papers': papers,
                'field': field
        ...
        # Store the papers and field in session for future use
        if papers:
            session['last_search_papers'] = papers
        if field:
            session['last_search_field'] = field
        
        # Validate we have enough papers
        if not papers or len(papers) < 3:
            return jsonify({
                'success': False,
                'error': 'Not enough papers to analyze. Please search for and add at least 3 papers first.'
            })
            
        logger.info(f'Finding research gaps in field: {field} with {len(papers)} papers')
        ...
        return jsonify({
            ...
            'research_opportunities': gaps_analysis.get('research_opportunities', []),
            'field': field
        })
...
def chat():
    ...
    try:
        ...
        papers = data.get('papers') if data and 'papers' in data else session.get('last_search_papers')
        field = data.get('field') if data and 'field' in data else session.get('last_search_field')
