...
def search_semantic_scholar(keywords, max_results=10, enable_ai=True, sort_by_citations=True):
    """
    ...
    If sort_by_citations is True, results will be sorted by citation count in descending order.
    """
    try:
        # Initialize API
        sch = SemanticScholar()
        ...
        for i, paper in enumerate(results):
            try:
                ...
                logger.info(f"Processed paper {i+1}: {paper_entry['title'][:50]}... (Citations: {citation_count})")
        ...
        # Sort papers by citation count if requested
        if sort_by_citations and processed_papers:
            logger.info("Sorting papers by citation count")
            processed_papers = sorted(processed_papers, key=lambda x: x.get('citation_count', 0), reverse=True)
            logger.info("Papers sorted by citation count")
    ...
    if not query:
        ...
        try:
            ...
            if max_results < 1 or max_results > 20:
                ...
                sort_option = input("Sort by citations? (y/n): ").strip().lower()
                sort_by_citations = sort_option.startswith('y')
                
                papers = search_semantic_scholar(query, max_results, sort_by_citations=sort_by_citations)
                if papers:
                    ...
                    print(f"\nFound {len(paper_list)} relevant papers:\n")
